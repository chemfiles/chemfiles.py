...
class Frame(object):
    ...
    def __init__(self, natoms=0):
        ...
        self._handle_ = self.c_lib.chrp_frame(c_size_t(natoms))
        _check_handle(self._handle_)

    def __del__(self):
        self.c_lib.chrp_frame_free(self._handle_)
    ...
    def atom(self, index):
        ...
        atom._handle_ = self.c_lib.chrp_atom_from_frame(
    ...
    def natoms(self):
        ...
        res = c_size_t()
        self.c_lib.chrp_frame_atoms_count(self._handle_, res)
        return res.value
    ...
    def positions(self):
        ...
        self.c_lib.chrp_frame_positions(self._handle_, res, c_size_t(natoms))
        return res
    ...
    def set_positions(self, positions):
        ...
        self.c_lib.chrp_frame_set_positions(
            self._handle_,
            positions,
            c_size_t(shape[0])
        )
    ...
    def velocities(self):
        ...
        natoms = self.natoms()
        res = np.zeros((natoms, 3), np.float32)
        self.c_lib.chrp_frame_velocities(self._handle_, res, c_size_t(natoms))
        return res
    ...
    def set_velocities(self, velocities):
        ...
        self.c_lib.chrp_frame_set_velocities(
            self._handle_,
            velocities,
            c_size_t(shape[0])
        )
    ...
    def has_velocities(self):
        ...
        res = c_bool()
        self.c_lib.chrp_frame_has_velocities(self._handle_, byref(res))
        return res.value

    def cell(self):
        '''Get the `UnitCell` from the ``Frame``'''
        cell = UnitCell(0, 0, 0)
        self.c_lib.chrp_cell_free(cell._handle_)
        cell._handle_ = self.c_lib.chrp_cell_from_frame(self._handle_)
        _check_handle(cell._handle_)
        return cell

    def set_cell(self, cell):
        '''Set the `UnitCell` of the `Frame`'''
        self.c_lib.chrp_frame_set_cell(self._handle_, cell._handle_)

    def topology(self):
        '''Get the ``Topology`` from the ``Frame``'''
        topology = Topology()
        self.c_lib.chrp_topology_free(topology._handle_)
        topology._handle_ = self.c_lib.chrp_topology_from_frame(self._handle_)
        _check_handle(topology._handle_)
        return topology

    def set_topology(self, topology):
        '''Set the `Topology` of the `Frame`'''
        self.c_lib.chrp_frame_set_topology(self._handle_, topology._handle_)

    def step(self):
        '''Get the ``Frame`` step, i.e. the frame number in the trajectory'''
        res = c_size_t()
        self.c_lib.chrp_frame_step(self._handle_, byref(res))
        return res.value

    def set_step(self, step):
        '''Set the ``Frame`` step'''
        self.c_lib.chrp_frame_set_step(self._handle_, c_size_t(step))
    ...
    def guess_topology(self, bonds=True):
        ...
        '''
        self.c_lib.chrp_frame_guess_topology(self._handle_, c_bool(bonds))
